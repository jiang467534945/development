...
!function ($) {
	...
	bTabs.prototype.addTab = function(id,title,url,loginCheck){
		...
		if(openTabs && $.isArray(openTabs) && openTabs.length>0){
			...
			//若功能已存在，则直接切换
			if(exist){
                $('#tabfont a[href="#'+tabId+'"]',$tabs).tab('show');
				return;
			}
		}else openTabs = new Array();
		$('#tabfont',$tabs).append('<a href="#'+tabId+'" data-toggle="tab">'+title+c.closeBtnTemplate+'</a>');
		var content = $('<div class="page-content-wrap" id="'+tabId+'"></div>');
		$('.page-content',$tabs).append(content);
		//切换到新增加的tab上
		$('#tabfont a',$tabs).tab('show');
		openTabs.push(tabId);
